...
public class TabGenerator
{
    ...
    private static PlotModel CreatePlot(string title, int[] x, double[] y, double[] y2, string axis2, string axis = "Time, ms")
    {
        var model = new PlotModel { Title = title };
        Legend legend = new Legend();
        legend.LegendOrientation = LegendOrientation.Horizontal;
        ...
        model.Legends.Add(legend);
        model.Axes.Add(new LinearAxis { Position = AxisPosition.Left, Title = axis, AxislineColor = OxyColors.Red});
        model.Axes.Add(new LinearAxis { Position = AxisPosition.Bottom, Title = axis2 });
        var series1 = new LineSeries
        {
            Title = "Результаты для встроенной",
            Color = OxyColors.Blue
    ...
    public static TabItemModel GenerateTabForQueueTextAnalyzer(int minN, int maxN, CommandMode mode, string axis2)
    {
        ...
        if (mode == CommandMode.Balanced)
        {
            y = Benchmark.RunRange(minN, maxN, () => new CustomTextAnalyzer(), mode);
            y2 = Benchmark.RunRange(minN, maxN, () => new BuiltInTextAnalyzer(), mode);
        }
        else
        {
        ...
        return new TabItemModel(title, CreatePlot(title, x, y, y2, "Attempt Number","Time, ms"));
    }
    public static TabItemModel GenerateTabForQueueTextAnalyzer(int minN, int maxN, CommandMode mode)
    {
        if (mode != CommandMode.Balanced)
        {
            minN /= 10;
